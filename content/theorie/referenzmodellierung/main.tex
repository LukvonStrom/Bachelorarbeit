...
\Todo{Einleitung -> Bass,Kazman --> Referenzmodell Basis von Refernzarchitektur}

Nach \citeauthor{Bass.2010} ist eine Referenzarchitektur ein spezialisiertes Referenzmodell, wie in \autoref{abb:RelationshipsReferenceModel} welches in Softwarearchitekturen instanziiert werden kann. \footcite[Vgl.][S.~17~f.]{Bass.2010} In diesem Kapitel soll deshalb speziell auf die theoretische Definition von Referenzmodellierung eingegangen werden und entsprechend die theoretische Definition des Referenzarchitekturbegriffs genauso wie mögliche Vorgehensmodelle betrachtet werden.
...
\subsection{Konstruktionstechniken}

\begin{figure}[H]
\centering
\includegraphics[height=0.5\textheight]{graphics/anwendung-referenzmodelle.pdf}
...
\subsection{Anwendung von Referenzmodellen}
Instanziierung
Konfiguration
...
\citeauthor{Muller.2020} empfiehlt eine Referenzarchitektur zur Generalisierung von vorhandenen Architekturen zu verwenden, schläg t aber für neue Technologien und Applikationen, die bislang in der Form kaum verwendet wurden ein inkrementelles Vorgehen vor.\footcite[Vgl. auch im Folgenden][7]{Muller.2020} Das inkrementelle Vorgehen von Muller beinhaltet dabei die Erstellung von Prototypen und inkrementelle Einholung von Feedback der Zielstakeholder.
...
Sehr wichtig für eine Referenzarchitektur ist auch die Dokumentation, wie die Wiederverwendung zu handhaben ist. Ein möglicher Ansatz wäre dabei die gezielte Integration und Dokumentation von Variationspunkten, wie von \citeauthor{Webber.2001} vorgeschlagen.\footcite[Vgl.][24\psqq]{Webber.2001} Mittels der Variationspunkte kann eine statische Referenz-/ Produktlinienarchitektur konstruiert werden, welche an spezifisch definierten Punkten angepasst werden muss, um einzigartige Architekturen zu erzeugen.\footcite[Vgl.][24]{Webber.2001} Dabei gibt es vier verschiedene Ansichten, aus denen Variationspunkte definiert werden können:\footcite[Vgl.][25\psq]{Webber.2001}
\begin{enumerate}
