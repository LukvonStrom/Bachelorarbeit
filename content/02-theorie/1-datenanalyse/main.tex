...
Es ist von zwei vorliegenden Typen von Zeitreihendaten auszugehen. Zum einen existiert der mit Zeitstempel versehene Messwert, welcher einen oder in manchen Fällen auch mehrere diskrete Messwerte kombiniert mit dem Zeitstempel der Erfassung übermittelt. Zum anderen existieren auch Ereignisse, welche keine Messwerte enthalten, sondern Ergebnisse einer Vorauswertung innerhalb des übermittelnden Systems sind. So wäre ein Ereignis beispielsweise niedriger Batteriestand eines verbundenen Sensors. Abhängig von der Vorauswertung werden Ereignisse einmal, nach Auftreten mit Zeitstempel oder periodisch mit Zeitstempel bis zum beheben der Ursache versendet. \Todo{belegen}
...
Eine der wichtigsten statistischen Lageparameter sind die Quantile eines Datensatzes und der Median, welcher ein spezielles, 50 prozentiges Quantil ist. Innerhalb eines gewissen Betrachtungsfensters können für Zeitreihendaten, wie für viele andere Datensätze auch, Quantile und der Median erfasst werden. Das Quantil erfasst mit der jeweiligen Prozentangabe und dem zugehörigen, ermittelten Wert die Grenze an Werten, die kleiner sind. Bei einem 75-\%-Quantil von zwei wären entsprechend nur 25\% der sich im Datensatz befindlichen Daten größer als zwei. Da ein Median das 50-\%-Quantil ist, bildet der Median entsprechend die Grenze ab, bei welcher 50\% der WErte im Datensatz kleiner und der Rest größer ist. In \autoref{abb:quantile} sind das 25,50 und 75-\%-Quantil auf einer modifizierten Sinusfunktion gezeigt.
\begin{figure}[H]
\centering
\begin{tikzpicture}
    \begin{axis}[
    ...
    legend pos=outer north east]
        ...
        \addplot {1.2392493};
        \addplot {1.72939505};
        \legend{$\sin(x-1.6)+1$,25-\%-Quantil,Median (50-\%-Quantil),75-\%-Quantil}
    \end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}
    ...
    legend pos=outer north east]
        ...
        \legend{Normaler Verlauf ($sin(deg(x-1.6))+1$),Anomalie 1,Anomalie 2}
    \end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}
    ...
    legend pos=outer north east]
        ...
        \legend{Tageshöchstwerte von \$GME,gleitender Durchschnitt (Intervall: 2), gleitender Durchschnitt (Intervall: 3),gleitender Durchschnitt (Intervall: 4),gleitender Durchschnitt (Intervall: 5)};
    \end{axis}
\end{tikzpicture}
\caption{Gleitender Durchschnitt des Tageshöchstwertes während des Gamestop Gamma Squeezes}
...
\caption[Aufbau und Ablauf in einer Data Pipeline]{Aufbau und Ablauf in einer Data Pipeline}
